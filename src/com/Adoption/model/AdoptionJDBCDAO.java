...
public class AdoptionJDBCDAO implements AdoptionDAO_Interface{
	...
	private static final String INSERT_ADOPT = 
			"			\"INSERT INTO ADOPTION (ADOPT_ID,ADOPT_SPECIES,ADOPT_SPONSOR,ADOPT_BTIME,ADOPT_DES) values ('A'||LPAD(to_char(adoption_seq.NEXTVAL),9,'0'),?,?,to_char(ADOPT_BTIME,'yyyy-mm-dd hh24:mi:ss')ADOPT_BTIME,?,?)" + 
			"";
	...
	public void insert(AdoptionVO adoptionVO) {
		...
		try {
			...
			pstmt.setString(1, adoptionVO.getAdopt_id());
			pstmt.setString(2, adoptionVO.getAdopt_species());
			pstmt.setString(3, adoptionVO.getAdopt_sponsor());
			pstmt.setString(4, adoptionVO.getAdopt_status());
			pstmt.setString(5, adoptionVO.getAdopt_apply_status());
			pstmt.setTimestamp(6, adoptionVO.getAdopt_btime());
			pstmt.setTimestamp(7, adoptionVO.getAdopt_etime());
			pstmt.setString(8, adoptionVO.getAdopt_des());
			pstmt.setBytes(9, adoptionVO.getAdopt_img());
	...
	public AdoptionVO findByPrimaryKey(String adopt_id) {
		...
		try {
			...
			while (rs.next()) {
				...
				adoptionVO.setAdopt_sponsor(rs.getString("adopt_status"));
	...
	public List<AdoptionVO> getAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				adoptionVO.setAdopt_sponsor(rs.getString("adopt_status"));
				adoptionVO.setAdopt_status(rs.getString("adopt_status"));
				adoptionVO.setAdopt_apply_status(rs.getString("adiot_apply_status"));
				adoptionVO.setAdopt_btime(rs.getTimestamp("adopt_btime"));
				adoptionVO.setAdopt_etime(rs.getTimestamp("adopt_etime"));
				adoptionVO.setAdopt_des(rs.getString("adopt_des"));
				adoptionVO.setAdopt_img(rs.getBytes("adopt_img"));
...
//		adoptionVO1.setAdopt_id("A000000009");
		adoptionVO1.setAdopt_species("羊咩咩");
		adoptionVO1.setAdopt_sponsor("M000000009");
//		adoptionVO1.setAdopt_status("未上架");
//		adoptionVO1.setAdopt_apply_status("未審核");
		adoptionVO1.setAdopt_btime(java.sql.Timestamp.valueOf("1995-03-15 16:00:00"));
		adoptionVO1.setAdopt_etime(null);
